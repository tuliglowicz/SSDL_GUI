...
function View(id, width, height, gui) {
			...
			tooltip = {
				...
				open: function open(title, text, x, y, evt) {
					...
					if (title && text) {
						...
						if (evt.shiftKey) this.openHelper(this.title, this.text, this.x, this.y);
						else this.tOut = setTimeout((function() {
	...
	function preloader(divId) {
		var $divElem = $("#" + divId + "_" + pf),
			...
			temp = {
				...
				init: function init() {
					var newDiv = "<div id=\"preloader_" + pf + "\" style=\" position:absolute; top:" + this.top + "px; left:" + this.left + "px; width:" + this.width + "px; height:" + this.height + "px; background-color: black; opacity: .5; z-index: 5\">" + "<img src=\"preloader.gif\" style=\" position:relative; top:" + this.imgTop + "px; left:" + this.imgLeft + "px\"></img></div>";
		...
		var tmp = {
			...
			draw: function draw() {
				...
				var onDblClick = function onDblClick(nodeType) {
						return function() {
							if (gui.controler) var label = prompt("Enter a label for the new node:");
							if (label) gui.controler.reactOnEvent("AddBlankNode", {
								label: label,
								nodeType: nodeType
							});
						}
				...
				var repo_service = this.paper.rect(nodeHorizontalPosition, 20, nodeLength, nodeHeight, 5).attr({
					fill: "#fbec88"
				...
				var repo_functionality = this.paper.rect(nodeHorizontalPosition, 90, nodeLength, nodeHeight, 5).attr({
					fill: "#a6c9e2"
				...
				var text_mediator = this.paper.text(textHorizontalPosition, 150, "Mediator").hide();
				// text_mediator.node.setAttribute("class","repository_text"));
				var repo_mediator = this.paper.rect(nodeHorizontalPosition, 600, nodeLength, nodeHeight, 5).attr({
					fill: "white"
				}).dblclick(onDblClick("Mediator")).hide();
	...
	function drawBottomBar(paper) {
		...
		var top = (paper.height * .95 >= 250) ? paper.height * .95 : 250,
			...
			result = {
				...
				createBar: function createBar(x, y, width, height) {
					...
					this.bar = paper.rect(x, y, width, height).attr({
						fill: "grey",
						opacity: this.invisible
					}).mouseover(function() {
						that.isVisible = true;
						...
						that.triangle1.animate({
							opacity: invisible
						}, that.animationTime);
						that.triangle2.animate({
							opacity: invisible
						}, that.animationTime);
						$.each(that.groups, function() {
							if (this.isVisible) {
								this.graphic.show().animate({
									opacity: visible
								}, that.animationTime);
								$.each(this.buttons, function() {
									if (this.isVisible) {
										this.graphic[0].show().animate({
											opacity: visible
										}, that.animationTime);
										this.graphic[1].show().animate({
											opacity: visible
										}, that.animationTime);
										this.graphic[2].show();
									}
							...
							if (!that.bar.isPointInside(x - offsetX, y - offsetY)) {
								...
								$.each(that.groups, function() {
									this.graphic.animate({
										opacity: that.invisible
									}, that.animationTime).hide();
									$.each(this.buttons, function() {
										this.graphic[0].animate({
											opacity: invisible
										}, that.animationTime).hide();
										this.graphic[1].animate({
											opacity: invisible
										}, that.animationTime).hide();
										this.graphic[2].hide();
									});
					...
					var that = this,
						...
						result = {
							...
							hideButton: function hideButton(label) {
								$.each(this.buttons, function() {
									if (this.label.toUpperCase() === label.toUpperCase()) this.hideThisButton();
								});
							...
							showButton: function showButton(label) {
								$.each(this.buttons, function() {
									if (this.label.toUpperCase() === label.toUpperCase()) this.showThisButton();
								});
							...
							createGraphic: function createGraphic() {
								var temp, bbox;
								temp = paper.text(0, this.y + 5, this.label).attr({
									"font-size": 10,
									fill: "black",
									opacity: 0
								});
				...
				addOption: function addOption(groupLabel, label, click, description) {
					var result = {
						...
						createGraphic: function createGraphic() {
							...
							temp1 = paper.text(0, 0, this.label).attr({
							...
							temp2 = paper.rect(this.x, this.y, this.width, this.height, 3).attr({
								fill: "ivory",
								opacity: invisible
							});
							...
							cover = paper.rect(this.x, this.y, this.width, this.height, 3).attr({
								"cursor": "pointer",
								fill: "red",
								opacity: 0.0
				...
				addSeparator: function addSeparator(x, y, h) {
					var sep = paper.rect(x, y, 1, h).attr({
						"stroke-width": "0",
						fill: "gray",
						opacity: invisible
					});
				...
				relocate: function relocate() {
					...
					$.each(this.groups, function() {
						if (this.isVisible) {
							...
							if (sum > 10) that.addSeparator(this.x, this.y, this.height);
							sum += this.width;
						}
					});
					...
					if (this.isVisible) $.each(this.separators, function() {
						this.show().animate({
							opacity: visible
						}, that.animationTime);
					});
		...
		result.pathString(
		parseInt(left + offset), parseInt(top + offset)));
		result.triangle2 = result.createTriangle(
		result.pathString(
		parseInt(width - 2 * offset), parseInt(top + offset)));

		var switchMode = function(arg) {
				return (function() {
					gui.controler.reactOnEvent("SwitchMode", {
						mode: arg
					})
				});
			};
		...
		var f3 = function() {
				alert("this is just for debugging")
			};
	...
	function form() {
		...
		var result = {
			...
			inputAndOutputAppender: function inputAndOutputAppender(input, id, number) {
				...
				$("#" + id).append("<tr id=\"" + tempId + "\" class=\"clickable\">" + "<td id=\"" + tempId + "_id\">" + input.id + "</td>" + "<td id=\"" + tempId + "_label\">" + input.label + "</td>" + "<td id=\"" + tempId + "_class\">" + input.class + "</td>" + "<td id=\"" + tempId + "_dataType\">" + input.dataType + "</td>" + "</tr>");
			},
			NFPropsAppender: function NFPropsAppender(input, number) {
				var tempId = "f_nonFunctionalDescriptionTabxNFProps" + number;
				$("#f_nonFunctionalDescriptionTab_NFProps tbody").append("<tr id=\"" + tempId + "\" class=\"clickable\">" + "<td id=\"" + tempId + "_weight\">" + input.weight + "</td>" + "<td id=\"" + tempId + "_name\">" + input.name + "</td>" + "<td id=\"" + tempId + "_relation\">" + input.relation + "</td>" + "<td id=\"" + tempId + "_unit\">" + input.unit + "</td>" + "<td id=\"" + tempId + "_value\">" + input.value + "</td>" + "</tr>");
			},
		...
		function(event) {
			...
			if (index == -1) alert("No input selected!");
			else {
		...
		function(event) {
			...
			if (index == -1) alert("No output selected!");
			else {
		...
		function(event) {
			...
			if (index == -1) alert("No entry selected!");
			else {
		...
		$('tr[id^="f_inputsTabxinputs"]').live("click", function(event) {
			...
			result.clearInputSelectionInTable();
			if (result.getSelectedInputIndex() == index) result.resetSelectedInputIndex();
			else {
		...
		$('tr[id^="f_outputsTabxoutputs"]').live("click", function(event) {
			var index = event.target.id.split("_")[1].split("").pop();
			result.clearOutputSelectionInTable();
			if (result.getSelectedOutputIndex() == index) result.resetSelectedOutputIndex();
			else {
		...
		$('tr[id^="f_nonFunctionalDescriptionTabxNFProps"]').live("click", function(event) {
			var index = event.target.id.split("_")[1].split("").pop();
			result.clearNFPropertySelectionInTable();
			if (result.getSelectedNFPropertyIndex() == index) result.resetSelectedNFPropertyIndex();
			else {
	...
	function nodeVisualizator(view) {
		var outputObject = {
			...
			getBlankNode: function getBlankNode(x, y) {
				var blankNode = {
					...
					prepareNodeDescription: function prepareNodeDescription() {
						...
						if (data) {
							if (data.physicalDescription) {
								result = "<b>Service description:</b><br/> serviceName: " + data.physicalDescription.serviceName + "<br/>serviceGlobalId: " + data.physicalDescription.serviceGlobalId + "<br/>address: " + data.physicalDescription.address + "<br/>operation: " + data.physicalDescription.operation + "<br/><b>Non functional properties:</b><br/>";
							}
							if (data.nonFunctionalDescription) {
								for (var i = 0; i < data.nonFunctionalDescription.length; i++)
								result += "non functional property #" + i + ":<br/>weight: " + data.nonFunctionalDescription[i].weight + "<br/>name: " + data.nonFunctionalDescription[i].name + "<br/>relation: " + data.nonFunctionalDescription[i].relation + "<br/>unit: " + data.nonFunctionalDescription[i].unit + "<br/>value: " + data.nonFunctionalDescription[i].value;
							}
						}
					...
					prepareDescriptionForInput: function prepareDescriptionForInput(inputId) {
						...
						var result = "";
						if (inputToDescribe) {
							result = "class: " + inputToDescribe.class + "<br/>id: " + inputToDescribe.id + "<br/>label: " + inputToDescribe.label + "<br/>dataType: " + inputToDescribe.dataType + "<br/>properties: " + inputToDescribe.properties;
					...
					prepareDescriptionForOutput: function prepareDescriptionForOutput(outputId) {
						...
						var result = "";
						if (outputToDescribe) {
							result = "class: " + outputToDescribe.class + "<br/>id: " + outputToDescribe.id + "<br/>label: " + outputToDescribe.label + "<br/>dataType: " + outputToDescribe.dataType + "<br/>properties: " + outputToDescribe.properties;
						}
						return result;
					},
					...
					setBold: function setBold(flag) {
						if (flag) this.set[0].attr("stroke-width", "2px");
						else this.set[0].attr("stroke-width", "1px");
					},
					...
					isInside: function isInside(x1, y1, x2, y2) {
						return this.x + this.width > x1 && this.y + this.height > y1 && this.x < x2 && this.y < y2
					},
					...
					getPossiblePositionsOfConnectors: function getPossiblePositionsOfConnectors() {
						return [
						[this.x + this.width / 2, this.y], [this.x + this.width, this.y + this.height / 2], [this.x + this.width / 2, this.y + this.height], [this.x, this.y + this.height / 2]];
					},
			...
			draw_controlNode: function draw_controlNode(node) {
				...
				var c = view.paper.circle(node.x, node.y, node.r).attr({
					fill: "white"
				}),
					label = view.paper.text(node.x, node.y - 20, node.id).attr("fill", "#333"),
					input_length, output_length, i_tab = [],
					o_tab = [],
					multX = 1,
					multY = 1,
					x1, y1, x2, y2;
				...
				node.raph_label.dblclick(function() {
					gui.controler.reactOnEvent("EditNode", {
						nodeId: node.id
					});
				})
				...
				x2 = Math.abs(x1 - node.x);
				y2 = Math.abs(y1 - node.y);
				...
				for (var k = 0; k < input_length; k++) {
					...
					if (k < 4) {
						multX = ((k % 2) === 0) ? -1 : 1;
						multY = (k < 2) ? 1 : -1;
						currIO.node = view.paper.path("M " + parseInt(node.x + (x2 * multX) + ((multX > 0) ? 7 + k : -15 - k)) + " " + parseInt(node.y + (y2 * multY)) + " l 0 10 l 10 0 l 0 -10 l -5 5 z").attr({
							'fill': "white"
						});
						...
						multX = ((k % 2) === 0) ? -1 : 1;
						multY = (k < 6) ? 1 : -1;
						currIO.node = view.paper.path("M " + parseInt(node.x + (y2 * multX) + ((multX > 0) ? 7 + k : -15 - k)) + " " + parseInt(node.y + (x2 * multY)) + " l 0 10 l 10 0 l 0 -10 l -5 5 z").attr({
							'fill': "white"
						});
						currIO.node.node.setAttribute("class", node.id + " input " + currIO.id);
					}
				...
				for (var l = 0; l < output_length; l++) {
					...
					if (l < 4) {
						multX = ((l % 2) === 0) ? -1 : 1;
						multY = (l < 2) ? 1 : -1;
						currIO.node = view.paper.path("M " + parseInt(node.x + (x2 * multX) + ((multX > 0) ? 7 + l : -15 - l)) + " " + parseInt(node.y + (y2 * multY)) + " l 10 0 l 0 -5 l -5 -5 l -5 5 z").attr({
							'fill': "white"
						});
						...
						multX = ((l % 2) === 0) ? -1 : 1;
						multY = (l < 6) ? 1 : -1;
						currIO.node = view.paper.path("M " + parseInt(node.x + (y2 * multX) + ((multX > 0) ? 7 + l : -15 - l)) + " " + parseInt(node.y + (x2 * multY)) + " l 10 0 l 0 -5 l -5 -5 l -5 5 z").attr({
							fill: "white"
						});
						currIO.node.node.setAttribute("class", node.id + " output " + currIO.id);
					}
				...
				node.isInside = function(x1, y1, x2, y2) {
					return this.x + this.r > x1 && this.y + this.r > y1 && this.x - this.r < x2 && this.y - this.r < y2;
				}
				...
				var isStartNode;
				if (node.controlType && node.controlType.toLowerCase() == "#start") isStartNode = true;

				view.dragCFArrow(c, node, isStartNode);

				// view.dragDFArrow(node.inputs.map(function(i){ return i.node; }), node);
				view.dragDFArrow(node.outputs.map(function(o) {
					return o.node;
				}), node);
			...
			draw_serviceNode: function draw_serviceNode(node, paper, drawNotForRepo) {
				...
				var id = node.id,
					...
					i, j, k, l, input_length, output_length, iDist, oDist, serviceName = node.serviceName,
					shortenServiceName, serviceNameShown, maxLength = 25;
				node.mainShape = rect;
				node.raph_label = label;
				...
				for (var k = 0; k < input_length; k++) {
					node.inputs[k].node = paper.path("M " + parseInt(node.x + (k + 1) * iDist) + " " + parseInt(node.y - 10) + " l 0 10 l 10 0 l 0 -10 l -5 5 z").attr({
						'fill': "#fbec88"
					});
					node.inputs[k].node.node.setAttribute("class", node.id + " input " + node.inputs[k].id);
				}
				for (var l = 0; l < output_length; l++) {
					node.outputs[l].node = paper.path("M " + parseInt(node.x + (l + 1) * oDist) + " " + parseInt(node.y + node.height) + " l 0 5 l 5 5 l 5 -5 l 0 -5 z").attr({
						'fill': "#fbec88"
					});
				...
				if (!drawNotForRepo) {
					...
					if (serviceName) {
						shortenServiceName = serviceName.length > maxLength ? serviceName.substring(0, maxLength - 3) + "..." : serviceName, serviceNameShown = paper.text(node.x + node.width / 2, node.y + 25, shortenServiceName);
						serviceNameShown.node.setAttribute("class", name);
						serviceNameShown.attr({
							title: serviceName,
							cursor: "default"
						});
					}
			...
			draw_functionalityNode: function draw_functionalityNode(node) {
				var id = node.id,
					...
					input_length, output_length, i, j = 0,
					iDist, oDist;
				node.mainShape = rect;
				node.raph_label = label;
				...
				for (var k = 0; k < input_length; k++) {
					node.inputs[k].node = view.paper.path("M " + parseInt(node.x + (k + 1) * iDist) + " " + parseInt(node.y - 10) + " l 0 10 l 10 0 l 0 -10 l -5 5 z").attr({
						'fill': "#a6c9e2"
					});
					node.inputs[k].node.node.setAttribute("class", node.id + " input " + node.inputs[k].id);
				}
				for (var l = 0; l < output_length; l++) {
					node.outputs[l].node = view.paper.path("M " + parseInt(node.x + (l + 1) * oDist) + " " + parseInt(node.y + node.height) + " l 0 5 l 5 5 l 5 -5 l 0 -5 z").attr({
						'fill': "#a6c9e2"
					});
					node.outputs[l].node.node.setAttribute("class", node.id + " output " + node.outputs[l].id);
				}
			...
			addTooltips: function addTooltips(visualizedNode) {
				...
				visualizedNode.prepareNodeDescription().mainShape.mouseover(
				(function(that) {
					return function(evt, x, y) {
						view.tooltip.open(that.type + ":" + that.label, that.description, x, y, evt);
					};
				})(visualizedNode)).mouseout(close);
			}
		};
	...
	var outputView = {
		...
		updateNode: function updateNode(node) {
			var id = node.nodeId,
				newNode, oldNode, index, that = this;
				...
				$.each(this.current_graph_view.edgesDF, function(i, v) {
					...
					if (this && this.sourceId === id) {
						...
						if (io_tmp) {
							...
							this.remove();
							if (!~indexesToSplice.indexOf(i)) indexesToSplice.push(i);
						}
		...
		addNodeFromRepo: function addNodeFromRepo(node) {
			...
			if (visualizedNode) this.current_graph_view.nodes.push(visualizedNode.switchMode(this.mode));
		},
		...
		convertGraphViewToXML: function convertGraphViewToXML(humanFriendly) {
			...
			if (!humanFriendly) stringXML = stringXML.replace(/\t/g, "").replace(/\n/g, "");
		...
		dragNodes: function drag(element, node) {
			var lastDragX, lastDragY, ox, dx, oy, dy, width, height, rWidth = gui.view.paper.width,
				rHeight = gui.view.paper.height,
				bbox, ctrl, transX, transY, flag = true,
				ready2move = false,
				that = this,
				...
				start = function start(x, y, evt) {
					...
					flag = false;
					if (!node.highlighted) {
						if (!evt.ctrlKey) gui.controler.reactOnEvent("DESELECT");
			...
			} else element.drag(move, start, stop);
		},
		dragCFArrow: function dragArrow(element, node, isStartNode) {
			var arrow, cx, cy, offsetX, offsetY, sourceNode, bbox, start = function start() {
					// if(gui.view.mode === "CF"){
				...
				stop = function(event) {
					...
					if (gui.view.mode === "CF") {
						...
						if (targetNode && sourceNode && targetNode.id !== sourceNode.id) gui.controler.reactOnEvent("AddCFEdge", {
							source: sourceNode,
							target: targetNode,
		...
		dragDFArrow: function dragArrow(element, node) {
			var arrow, cx, cy, offsetX, offsetY, sourceNode, targetId, output, bbox, glows = [],
				...
				stop = function stop(event) {
					...
					if (output && sourceNode && resultObj && !gui.view.isInputConnected(resultObj.targetId, resultObj.input.id)) {
						if (resultObj.input.dataType === output.dataType) {
							...
							gui.controler.reactOnEvent("Error", {
								msg: "You tried to make connection between input and output od different data types"
							})
						}
					} else {}
					$.each(glows, function() {
			...
			if (foundedEdge) {
				...
				var edgeObject = {
					...
					update: function() {
						...
						this.source.getPossiblePositionsOfConnectors(), this.target.getPossiblePositionsOfConnectors());
						try {
							this.arrow[0].remove();
							this.arrow[1].remove();
						} catch (e) {
		...
		getDFEdge: function getEdge(sourceId, targetId, outputId, inputId) {
			var foundedDFEdge;
			$.each(this.current_graph_view.edgesDF, function() {
				if (this.sourceId === sourceId && this.targetId === targetId && this.input.id === inputId && this.output.id === inputId) {
		...
		drawGraph: function drawGraph(graph_json) {
			...
			if (!this.paper) {
				...
				$.each(graph_json.nodes, function(key, val) {
					...
					if (visualizedNode) graph_view.nodes.push(visualizedNode);
				});
				...
				$.each(graph_json.nodes, function(key, val) {
					...
					$.each(val.sources, function() {
						...
						if (tmp) graph_view.edgesCF.push(tmp);
					});
					$.each(val.functionalDescription.inputs, function() {
						if (this && this.source && this.source.length == 2) {
							...
							if (tmp) graph_view.edgesDF.push(tmp);
						}
					});
				});
		...
		getBestConnectors: function getBestConnectors(sourceConnectors, targetConnectors) {
			var minOdl = Infinity,
				minI, minJ, dx, dy, dz, i, iMax, j, jMax;
		...
		init: function init() {
			...
			this.bgSelectionHelper = this.paper.rect(0, 0, width, height).attr({
				fill: "#DEDEDE",
				stroke: "none"
			}).toBack();
		...
		setBold: function setBold(x1, y1, x2, y2) {
			$.each(this.current_graph_view.nodes, function(k, v) {
				if (v.isInside(x1, y1, x2, y2)) v.setBold(true);
				else v.setBold(false);
			});
		},
		...
		selectNodesInsideRect: function selectNodesInsideRect(x1, y1, x2, y2, ctrl) {
			//alert(x1+":"+x2+":"+ctrl)
			$.each(this.current_graph_view.nodes, function(k, v) {
				if (v.isInside(x1, y1, x2, y2)) v.highlight(ctrl);
				else if (!ctrl) {
					v.removeHighlight();
				}
			...
			if (x1 && y1) {
				...
				$.each(this.current_graph_view.nodes, function() {
					...
					if (this.isInside(x1, y1, x2, y2)) {
						...
						if (resultTab.length >= count) return false;
					}
				});
			} else {
				// console.trace();
		...
		getInputByPosition: function getInputByPosition(x, y) {
			...
			var result, bbox, loopControler = true;
			$.each(this.current_graph_view.nodes, function(k, v) {
				$.each(v.inputs, function() {
					bbox = this.node.getBBox();
					if (bbox.x + bbox.width > x && bbox.y + bbox.height > y && bbox.x < x && bbox.y < y) {
						result = {
							targetId: v.id,
							input: this
						};
		...
		removeCurrentGraph: function showCurrentGraph() {
			...
			if (this.mode === "DF") $.each(this.current_graph_view.edgesDF, function() {
				this.remove();
			});
			else if (this.mode === "CF") $.each(this.current_graph_view.edgesCF, function() {
				this.remove();
			});
		},
		removeAllGraphs: function showCurrentGraph() {
			...
			$.each(this.graph_views_tab, function() {
				...
				if (that.mode === "DF") $.each(this.edgesDF, function() {
					this.remove();
				});
				else if (that.mode === "CF") $.each(this.edgesCF, function() {
					this.remove();
				});
			});
	...
	outputView.mainMenu.addOption("File", "America", function() {
		console.log("duuuupa")
	}, "");
	outputView.mainMenu.addSubOption("File", "America", "Fuck", function() {
		console.log("duuuupa")
	}, "L+A");
	outputView.mainMenu.addSubOption("File", "America", "yea", function() {
		console.log("duuuupa")
	}, "L+A");
	outputView.mainMenu.addOption("File", "Open", function() {
		console.log("duuuupa")
	}, "Ctrl+O");
	outputView.mainMenu.addSubOption("File", "Open", "yea", function() {
		console.log("duuuupa")
	}, "L+A");
	outputView.mainMenu.addSeparator("File");
	outputView.mainMenu.addOption("File", "Exit", function() {
		console.log("duuuupa")
	}, "Alt+F4");
	outputView.mainMenu.addOption("Edit", "Undo", function() {
		alert("programuje hardo!")
	}, "Ctrl+Z");
	outputView.mainMenu.addSubOption("Edit", "Undo", "yea", function() {
		console.log("duuuupa")
	}, "L+A");
	outputView.mainMenu.addOption("Edit", "Redo", function() {
		alert("programuje hardo!")
	}, "Ctrl+Y");
	outputView.mainMenu.addSubOption("Edit", "Redo", "yea", function() {
		console.log("duuuupa")
	}, "L+A");
	outputView.mainMenu.addSubOption("Edit", "Undo", "noooooo", function() {
		console.log("duuuupa")
	}, "L+A");

	outputView.MenuList.getInstance().push(outputView.mainMenu);

	var lastDragX, lastDragY, ox, dx, oy, dy, width, height, lastRot = 0,
		itWasJustAClick = false,
		sel, x1, x2, y1, y2, bgStart = function(x, y, d) {
			itWasJustAClick = true;
			ox = d.layerX;
			oy = d.layerY;
			lastDragX = 0;
			lastDragY = 0;
			sel = outputView.paper.rect(ox, oy, 1, 1).attr({
				fill: "#aaaaff",
				stroke: "#0000ff",
				opacity: .2
			});
		...
		bgMove = function(x, y, evt) {
			...
			if (rot == 0 || rot == -180) sel.attr("width", (x < 0 ? -x : x) + "px").attr("height", (y < 0 ? -y : y) + "px");
			else sel.attr("height", (x < 0 ? -x : x) + "px").attr("width", (y < 0 ? -y : y) + "px");
			...
			if (lastDragX >= 0) x2 += lastDragX;
			else x1 += lastDragX;

			if (lastDragY >= 0) y2 += lastDragY;
			else y1 += lastDragY;
