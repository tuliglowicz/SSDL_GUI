function startstopNode(cx, cy, name, class_, nodetype, sources, address, description, thisGraph){
	this.super = Node	
	...
	this.drawElement = function(){
		this.c = thisObj.paper.circle(this.cx, this.cy, 15).attr("fill","r(0.5, 0.05)#fff-#aaa-#000");
		
		
		this.c.node.setAttribute("class",name);
		this.c.click(function(){raport(thisObj.toString())});
				
		if(name == undefined || name == "null") name = "<tmp>";
		else name = (name.length > 9 ? name.substring(0, 7)+"..." : name)
		this.label = thisObj.paper.text(this.cx, this.cy-20, name).attr("fill", "#333");
		this.label.node.removeAttribute("style");
		this.label.node.removeAttribute("text");
		this.label.node.setAttribute("class","label");
		this.label.node.setAttribute("class",name);
		...
		var move2 = function(a, b, c, d, event){
			...
			arrow = thisObj.paper.arrow(ccx, ccy, event.clientX-ofsetX, event.clientY - ofsetY, 4);
		}
		...
		var stop2 = function(event){
			...
			if(elemName != null && elemName != name){
				myfound = thisObj.g.nodes.myFind(elemName);
				//alert(myfound)
				if(myfound != null && myfound.sources.push2(name)) //push2 zwraca true, jeśli dodał coś
					thisObj.g.addEdge(name, elemName)
				else{
				}
